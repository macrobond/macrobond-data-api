...
import os as _os
from typing import (
    TYPE_CHECKING as __TYPE_CHECKING,
    Dict as _Dict,
...
from .common.enums import SeriesFrequency, SeriesWeekdays, CalendarMergeMode
...
if __TYPE_CHECKING:
    from macrobond_data_api.common.api import Api as _Api

__MACROBOND_DATA_API_CURRENT_API: _Optional["_Api"] = None
...
def metadata_list_values(name: str) -> "MetadataValueInformation":
    ...
    ```
    """
    return __get_api().metadata_list_values(name)


def metadata_get_attribute_information(*names: str) -> _List["MetadataAttributeInformation"]:
    # fmt: off
    """
    ...
    *names : str
    ...
    """
    # fmt: on
    return __get_api().metadata_get_attribute_information(*names)


def metadata_get_value_information(*name_val: _Tuple[str, str]) -> _List["MetadataValueInformationItem"]:
    # fmt: off
    """
    ...
    *name_val : Tuple[str, str]
    ...
    """
    # fmt: on
    return __get_api().metadata_get_value_information(*name_val)


# revision


def get_revision_info(*series_names: str, raise_error: bool = None) -> _List["RevisionInfo"]:
    """
    ...
    *series_names : str
    ...
    """
    return __get_api().get_revision_info(*series_names, raise_error=raise_error)


def get_vintage_series(time: _datetime, *series_names: str, raise_error: bool = None) -> _List["VintageSeries"]:
    """
    ...
    """
    return __get_api().get_vintage_series(time, *series_names, raise_error=raise_error)


def get_nth_release(nth: int, *series_names: str, raise_error: bool = None) -> _List["Series"]:
    """
    ...
    The result is in the same order as in the request.
    """
    return __get_api().get_nth_release(nth, *series_names, raise_error=raise_error)
...
def get_all_vintage_series(series_name: str) -> "GetAllVintageSeriesResult":
    ...
    """
    return __get_api().get_all_vintage_series(series_name)


def get_observation_history(series_name: str, *times: _datetime) -> _List["SeriesObservationHistory"]:
    """
    ...
    """
    return __get_api().get_observation_history(series_name, *times)
...
def entity_search(
    text: str = None,
    entity_types: _Union[_Sequence[str], str] = None,
    must_have_values: _Dict[str, object] = None,
    must_not_have_values: _Dict[str, object] = None,
    must_have_attributes: _Union[_Sequence[str], str] = None,
    must_not_have_attributes: _Union[_Sequence[str], str] = None,
    ...
    """
    return __get_api().entity_search(
        text,
        entity_types,
        must_have_values,
...
def entity_search_multi_filter(
    ...
    *filters : `macrobond_data_api.common.types.search_filter.SearchFilter`
    ...
    `macrobond_data_api.common.types.search_result.SearchResult`
    """
    return __get_api().entity_search_multi_filter(
...
def get_one_series(series_name: str, raise_error: bool = None) -> "Series":
    ...
    """
    return __get_api().get_one_series(series_name, raise_error)


def get_series(*series_names: str, raise_error: bool = None) -> _List["Series"]:
    """
    ...
    *series_names : str
    ...
    The result is in the same order as in the request.
    """
    return __get_api().get_series(*series_names, raise_error=raise_error)
...
def get_one_entity(entity_name: str, raise_error: bool = None) -> "Entity":
    ...
    """
    return __get_api().get_one_entity(entity_name, raise_error)


def get_entities(*entity_names: str, raise_error: bool = None) -> _List["Entity"]:
    """
    ...
    *entity_names : str
    ...
    The result is in the same order as in the request.
    """
    return __get_api().get_entities(*entity_names, raise_error=raise_error)


def get_unified_series(
    *series_entries: _Union["SeriesEntry", str],
    ...
    *series_entries : Union[macrobond_data_api.common.types.series_entry.SeriesEntry, str]
    ...
    The result is in the same order as in the request.
    """
    return __get_api().get_unified_series(
        *series_entries,
        frequency=frequency,
        weekdays=weekdays,
