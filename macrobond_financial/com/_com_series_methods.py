...
from typing import Optional, Sequence, Tuple, Any, Union, Dict, List, TYPE_CHECKING
...
def _raise_get_entities_error(
    _raise: bool,
    com_entitys: Union[Sequence["ComEntity"], Sequence["ComSeries"]],
    names: Sequence[str],
) -> None:
    if not _raise:
        return
    ...
    for i, entitie in enumerate(com_entitys):
        if entitie.IsError:
            entities.append(EntitieErrorInfo(names[i], entitie.ErrorMessage))

    if len(entities) != 0:
        raise GetEntitiesError(entities)


def _fill_metadata_from_entity(
    com_entity: "ComEntity", metadata: Union[dict, Any]
) -> None:
...
class _GetOneSeriesReturn(SeriesMethods.GetOneSeriesReturn):
    ...
    def object(self) -> Series:
        ...
        _raise_get_entities_error(self.__raise, [com_series], [self.__series_name])
...
class _GetSeriesReturn(SeriesMethods.GetSeriesReturn):
    ...
    def tuple_of_objects(self) -> Tuple[Series, ...]:
        ...
        _raise_get_entities_error(self.__raise, com_series, self.__series_names)
        ret: List[Series] = []
...
class _GetOneEntitieReturn(SeriesMethods.GetOneEntitieReturn):
    ...
    def object(self) -> Entity:
        ...
        _raise_get_entities_error(self.__raise, [com_entity], [self.__entity_name])
...
class _GetEntitiesReturn(SeriesMethods.GetEntitiesReturn):
    ...
    def tuple_of_objects(self) -> Tuple[Entity, ...]:
        ...
        _raise_get_entities_error(self.__raise, com_entitys, self.__entity_names)
        ret: List[Entity] = []
...
class _GetUnifiedSeriesReturn(SeriesMethods.GetUnifiedSeriesReturn):
    ...
    def check_for_error(self, com_series: Tuple["ComSeries", ...]) -> None:
        if self.__raise:
            entities: List[EntitieErrorInfo] = []
            for i, com_serie in enumerate(com_series):
                if com_serie.IsError:
                    entities.append(
                        EntitieErrorInfo(
                            self.__request.AddedSeries[i].Name, com_serie.ErrorMessage
                        )
                    )
    ...
    def object(self) -> UnifiedSeries:
        ...
        self.check_for_error(com_series)
...
class _ComSeriesMethods(SeriesMethods.SeriesMethods):
    ...
    def get_entities(
        ...
        return _GetEntitiesReturn(
            self.__database, entity_names, raise_get_entities_error
        )
        ...
        *series_entries: Union["SeriesEntrie", str],
        ...
        for entrie_or_name in series_entries:
            ...
            if entrie_or_name.missing_value_method is not None:
                series_expression.MissingValueMethod = (
                    entrie_or_name.missing_value_method
                )

            if entrie_or_name.to_lowerfrequency_method is not None:
                series_expression.ToLowerFrequencyMethod = (
                    entrie_or_name.to_lowerfrequency_method
                )
        ...
        return _GetUnifiedSeriesReturn(
            self.__database, request, raise_get_entities_error
        )
