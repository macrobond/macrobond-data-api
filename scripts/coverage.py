...
class Coverage(WorkItem):
    async def run(self) -> None:
        await self.python_run(
            "coverage",
            "erase",
            "run --omit=macrobond_data_api/common/enums/**,tests/** -m pytest -p no:faulthandler",
            "html",
            "report -m",
            "erase",
        )
...
class ComCoverage(WorkItem):
    async def run(self) -> None:
        await self.python_run(
            "coverage",
            "erase",
            "run --include=macrobond_data_api/com/com_api.py,macrobond-data-api/com/** -m pytest -p no:faulthandler",
            "html -d htmlcov_com",
            "report -m",
            "erase",
        )
...
class WebCoverage(WorkItem):
    async def run(self) -> None:
        await self.python_run(
            "coverage",
            "erase",
            "run --include=macrobond_data_api/web/web_api.py,macrobond_data_api/web/** -m pytest -p no:faulthandler",
            "html -d htmlcov_web",
            "report -m",
            "erase",
        )
